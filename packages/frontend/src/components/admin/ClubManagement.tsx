...
import ActivityManagement from '../club/ActivityManagement';
import ApplicationReview from '../club/ApplicationReview';
...
const ClubManagement: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
                          ...
                          {menuClubId === club.id && (
                            ...
                            >
                              ...
                              {activeTab === 'active' ? (
                                <>
                                  <button
                                    onClick={() => { setSelectedClub(club); setManageTab(0); setManageDialogOpen(true); setMenuClubId(null); }}
                                    className="w-full px-5 py-3.5 text-left hover:bg-[var(--bg-subtle)] transition-colors flex items-center gap-3 text-text-primary text-base font-medium border-b border-[var(--border)]"
                                  >
                                    <svg className="w-5 h-5 text-[var(--accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Manage Club Dialog - Full Screen */}
      <AnimatePresence>
        {manageDialogOpen && selectedClub && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-[var(--bg)] z-50 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)] bg-[var(--bg-elevated)] shrink-0">
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="p-2 rounded-lg hover:bg-[var(--bg-subtle)] transition-colors text-text-secondary hover:text-text-primary"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                  <h2 className="text-lg font-bold text-text-primary">{selectedClub.name}</h2>
                  <p className="text-xs text-text-tertiary">Admin — Klub İdarəetməsi</p>
                </div>
              </div>
              <button
                onClick={() => setManageDialogOpen(false)}
                className="p-2 rounded-lg hover:bg-[var(--bg-subtle)] transition-colors text-text-secondary"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* Tabs */}
            <div className="flex gap-1 px-6 pt-4 pb-0 border-b border-[var(--border)] bg-[var(--bg-elevated)] shrink-0">
              {[
                { label: 'Fəaliyyətlər', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
                { label: 'Müraciətlər', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
                { label: 'Üzvlər', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
                { label: 'Tənzimləmələr', icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065zM15 12a3 3 0 11-6 0 3 3 0 016 0z' },
              ].map((tab, i) => (
                <button
                  key={i}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {manageTab === 0 && <ActivityManagement clubId={selectedClub.id} />}
              {manageTab === 1 && <ApplicationReview clubId={selectedClub.id} />}
              {manageTab === 2 && <ClubMembers clubId={selectedClub.id} />}
              {manageTab === 3 && <ClubSettings club={selectedClub} />}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
